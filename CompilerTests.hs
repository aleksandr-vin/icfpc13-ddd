...
test13 = TestCase (assertEqual ""
  ...
  where foo = C.compile (Prog (Op1 Shr16 (P X)))

test14 = TestCase (assertEqual ""
                  [0,0,0,0]
                  $ map (foo)
                  [0,1,2,4])
  where foo = C.compile (Prog (Op2 And (P X) (P Zero)))
        
test15 = TestCase (assertEqual ""
                  [0,1,0,1,0,1,0]
                  $ map (foo)
                  [0,1,2,3,4,5,6])
  where foo = C.compile (Prog (Op2 And (P X) (P One)))
        
test16 = TestCase (assertEqual ""
                  [0,1,2,4]
                  $ map (foo)
                  [0,1,2,4])
  where foo = C.compile (Prog (Op2 Or (P X) (P Zero)))
        
test17 = TestCase (assertEqual ""
                  [1,1,3,3,5,5,7]
                  $ map (foo)
                  [0,1,2,3,4,5,6])
  where foo = C.compile (Prog (Op2 Or (P X) (P One)))
        
test18 = TestCase (assertEqual ""
                   [0,1,2,4,0xff,0xff0ff0,0xFFFFFFFFffffffff]
                   $ map (foo)
                   [0,1,2,4,0xff,0xff0ff0,0xFFFFFFFFffffffff])
  where foo = C.compile (Prog (Op2 Xor (P X) (P Zero)))
        
test19 = TestCase (assertEqual ""
                   [1,0,3,5,0xfe,0xff0ff1,0xFFFFFFFFfffffffe]
                   $ map (foo)
                   [0,1,2,4,0xff,0xff0ff0,0xFFFFFFFFffffffff])
  where foo = C.compile (Prog (Op2 Xor (P X) (P One)))
        
test20 = TestCase (assertEqual ""
                   [0,0,0,0,0x00,0x000000,0x0000000000000000]
                   $ map (foo)
                   [0,1,2,4,0xff,0xff0ff0,0xFFFFFFFFffffffff])
  where foo = C.compile (Prog (Op2 Xor (P X) (P X)))
...
test24 = TestCase (assertEqual ""
  ...
  where foo = C.compile (Prog (TFold (P' Y)))
